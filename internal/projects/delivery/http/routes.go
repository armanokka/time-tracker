...
func MapRoutes(projectsGroup *gin.RouterGroup, project projects.Handlers, task projects.TaskHandlers, mw middleware.Manager) {
	...
	projectsGroup.DELETE("/:project_id/users/:user_id", mw.OwnerOrAdminMiddleware(), project.RemoveMember())

	tasksGroup := projectsGroup.Group("/:project_id/tasks")
	tasksGroup.Use(mw.MemberOrAdminMiddleware())
	...
	tasksGroup.POST("/:task_id/users", task.AddMember())
	tasksGroup.DELETE("/:task_id/users/:user_id", task.DeleteMember())
}
